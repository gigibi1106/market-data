...
def get_sgx_taiwan():
    """用 Firecrawl stealth 抓取 investing.com 富台指數"""
    try:
        r = requests.post(
            ...
            json={
                ...
                'formats': ['json'],
                'jsonOptions': {
                    'prompt': 'Extract the current price (regularMarketPrice), previous close (previousClose), and change percentage (changePercentage). Return as JSON with those exact keys.'
                },
                'proxy': 'stealth',
                'waitFor': 5000
            },
        ...
        price = float(data.get('regularMarketPrice') or data.get('currentPrice') or 0)
        prev  = float(data.get('previousClose') or data.get('prevClose') or 0)
        pct   = float(data.get('changePercentage') or data.get('changePercent') or 0)

        # 若 pct 還是 0，從 price/prev 計算
        if pct == 0 and prev and abs(price - prev) > 0.01:
            pct = round((price - prev) / prev * 100, 2)

        print(f"SGX Taiwan: price={price}, prev={prev}, pct={pct}%")
        return {'price': price, 'prev': prev, 'chg_pct': pct}
